...
type AgentId = 'aoi' | 'lacia' | 'methode' | 'satonus' | 'snowdrop' | 'kouka';
...
const CLAWROOM_CHAT_URL = 'http://localhost:18790/api/chat';
const DEFAULT_AGENT: AgentId = 'aoi';
const AGENT_META: AgentMeta[] = [
  { id: 'aoi', label: 'Aoi', initial: 'A', color: '#2EA7FF', role: 'Dispatcher' },
  { id: 'lacia', label: 'Lacia', initial: 'L', color: '#7EC8E3', role: 'Strategy' },
  { id: 'methode', label: 'Methode', initial: 'M', color: '#FF8C42', role: 'Execution' },
  { id: 'satonus', label: 'Satonus', initial: 'S', color: '#FFD700', role: 'Review' },
  { id: 'snowdrop', label: 'Snowdrop', initial: 'W', color: '#E8E8E8', role: 'Research' },
  { id: 'kouka', label: 'Kouka', initial: 'K', color: '#E53E3E', role: 'Delivery' },
];
...
function getAgentMeta(agentId: AgentId): AgentMeta {
  return AGENT_META.find((item) => item.id === agentId) ?? AGENT_META[0];
}

function parseAgentPrefix(text: string): { targetAgent: AgentId | null; cleanedText: string } {
  const match = text.trim().match(/^@([a-zA-Z]+)\b\s*/);
  if (!match) {
    return { targetAgent: null, cleanedText: text };
  }
  const alias = match[1].toLowerCase();
  const target = AGENT_ALIAS_TO_ID[alias];
  if (!target) {
    return { targetAgent: null, cleanedText: text };
  }
  const cleanedText = text.trim().replace(/^@[a-zA-Z]+\b\s*/, '').trim();
  return { targetAgent: target, cleanedText };
}

async function chatViaClawRoom(agent: AgentId, message: string): Promise<string> {
  const response = await fetch(CLAWROOM_CHAT_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ agent, message }),
  });
  if (!response.ok) {
    throw new Error(`ClawRoom chat failed: HTTP ${response.status}`);
  }
  ...
  while (true) {
    ...
    for (const line of parts) {
      ...
      const payloadRaw = trimmed.slice(6);
      ...
      if (payload.type === 'content' && typeof payload.text === 'string') {
        ...
        throw new Error(payload.message || 'ClawRoom returned an error');
      }
    }
  }
...
const ChatPanel: React.FC<{
  ...
  const [activeAgent, setActiveAgent] = useState<AgentId>(DEFAULT_AGENT);

  // Session key for chat history isolation (character × mod)
  const sessionPathBase = buildSessionPath(charCollection.activeId, modCollection.activeId);
  const sessionPath = `${sessionPathBase}/agents/${activeAgent}`;
    ...
    loadChatHistory(sessionPath).then((data) => {
      ...
      if (loadedMessages.length === 0 && loadedHistory.length === 0) {
        ...
        if (data?.suggestedReplies?.length) {
          ...
          if (activeAgent === 'aoi' && onlyPrologue) {
            const entry = getActiveModEntry(modCollection);
            const openingReplies = entry.config.opening_rec_replies;
            setSuggestedReplies(
  ...
  }, [activeAgent, sessionPath, modCollection, seedPrologue]);
  ...
  const processActionQueue = useCallback(async () => {
    ...
    while (actionQueueRef.current.length > 0) {
      ...
      setLoading(true);
      try {
        await runConversation(newHistory, cfg, 'aoi');
      } catch (err) {
  ...
  useEffect(() => {
    ...
    return unsubscribe;
  }, [activeAgent, processActionQueue]);

  // Send message
  const handleSend = useCallback(
    async (overrideText?: string) => {
      const rawText = overrideText ?? input.trim();
      const { targetAgent: prefixedAgent, cleanedText } = parseAgentPrefix(rawText);
      const targetAgent = prefixedAgent ?? activeAgent;
      const text = (prefixedAgent ? cleanedText : rawText).trim();
      ...
      const newHistory: ChatMessage[] = [...chatHistory, { role: 'user', content: rawText }];
      setChatHistory(newHistory);

      setLoading(true);
      try {
        if (targetAgent === 'aoi') {
          await runConversation(newHistory, config, targetAgent);
        } else {
          const routedReply = await chatViaClawRoom(targetAgent, text);
          addMessage({
            id: String(Date.now()),
            role: 'assistant',
            content: routedReply || '(No response)',
            agentId: targetAgent,
          });
          setChatHistory((prev) => [...prev, { role: 'assistant', content: routedReply }]);
        }
      } catch (err) {
    ...
    [input, loading, config, chatHistory, addMessage, activeAgent],
  );

  // Core conversation loop
  const runConversation = async (history: ChatMessage[], cfg: LLMConfig, agentId: AgentId) => {
    await seedMetaFiles();
    ...
    <>
      ...
      >
        ...
        <div className={styles.chatSide}>
          ...
          <div className={styles.agentTabs} data-testid="agent-selector">
            {AGENT_META.map((agent) => (
              <button
                key={agent.id}
                className={`${styles.agentTab} ${activeAgent === agent.id ? styles.agentTabActive : ''}`}
                onClick={() => setActiveAgent(agent.id)}
                title={`${agent.label} · ${agent.role}`}
                style={{ ['--agent-accent' as string]: agent.color } as React.CSSProperties}
              >
                <span className={styles.agentTabInitial}>{agent.initial}</span>
                <span className={styles.agentTabLabel}>{agent.id === 'aoi' ? 'Aoi' : agent.initial}</span>
              </button>
            ))}
          </div>
          ...
          <div className={styles.messages} data-testid="chat-messages">
            {messages.length === 0 && (
              <div className={styles.emptyState}>
                {hasUsableLLMConfig(config)
                  ? `${activeAgentMeta.label} is ready to chat...`
            ...
            {messages.map((msg) => (
              <React.Fragment key={msg.id}>
                {(() => {
                  const messageAgent = getAgentMeta(msg.agentId ?? activeAgent);
                  return (
                <div
                  ...
                  style={
                    msg.role === 'tool'
                      ? undefined
                      : ({ ['--agent-accent' as string]: messageAgent.color } as React.CSSProperties)
                  }
                >
                  {msg.role !== 'tool' && (
                    <span className={styles.agentMessageBadge}>{messageAgent.initial}</span>
                  )}
          ...
          <div className={styles.inputArea}>
            <textarea
              ...
              placeholder="Type a message... (@agent optional)"
              rows={1}
              disabled={loading}
